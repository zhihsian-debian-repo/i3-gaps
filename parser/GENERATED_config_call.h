static void GENERATED_call(const int call_identifier, struct ConfigResultIR *result) {
    switch (call_identifier) {
         ...
         case 5:
             result->next_state = BAR;
#ifndef TEST_PARSER
             cfg_bar_workspace_min_width(&current_match, result, get_long("width"));
#else
       ...
       cfg_criteria_add(&current_match, result, get_string("ctype"), get_string("cvalue"));
         ...
         case 106:
             result->next_state = CRITERIA;
#ifndef TEST_PARSER
             cfg_criteria_init(&current_match, result, 32);
#else
       ...
       cfg_criteria_pop_state(&current_match, result);
         ...
         case 125:
             result->next_state = CRITERIA;
#ifndef TEST_PARSER
             cfg_criteria_init(&current_match, result, 70);
#else
           fprintf(stderr, "cfg_criteria_init(%d)\n", 70);
       cfg_criteria_init(&current_match, result, 70);
#endif
         ...
         case 126:
             result->next_state = CRITERIA;
#ifndef TEST_PARSER
             cfg_criteria_init(&current_match, result, 38);
#else
           fprintf(stderr, "cfg_criteria_init(%d)\n", 38);
       cfg_criteria_init(&current_match, result, 38);
#endif
